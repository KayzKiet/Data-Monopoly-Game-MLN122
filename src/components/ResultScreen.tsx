    ...
    return (
      <section className="screen-shell">
        <div className="panel mx-auto max-w-2xl text-center">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-gold">Result</p>
      ...
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="panel">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <ScoreCard label="Money" value={`$${winnerBreakdown.money}`} />
            <ScoreCard label="Asset value" value={`$${winnerBreakdown.assetValue}`} />
            <ScoreCard label="Users" value={winnerBreakdown.users} />
            <ScoreCard label="Data" value={winnerBreakdown.data} />
            <ScoreCard label="Theory points" value={winnerBreakdown.theoryPoints} />
            <ScoreCard label="Market power" value={`${winnerBreakdown.marketPower} MP`} highlight />
          </div>
      ...
      <div className="panel overflow-x-auto">
        ...
        <table className="mt-4 w-full min-w-[780px] text-left text-sm">
          <thead className="text-cyan">
            <tr>
              <th className="border-b border-white/10 py-3 pr-4">Rank</th>
              <th className="border-b border-white/10 py-3 pr-4">Player</th>
              <th className="border-b border-white/10 py-3 pr-4">Money</th>
              <th className="border-b border-white/10 py-3 pr-4">Assets</th>
              <th className="border-b border-white/10 py-3 pr-4">Users</th>
              <th className="border-b border-white/10 py-3 pr-4">Data</th>
              <th className="border-b border-white/10 py-3 pr-4">Theory</th>
              <th className="border-b border-white/10 py-3 pr-4">Market Power</th>
              <th className="border-b border-white/10 py-3">Final Score</th>
            </tr>
          </thead>
